...
print_test_case "[101] Behaviour: (UDP) Server stays alive after Client quits (${PYVER})"

# 1. Start Server in background
# 2. Start Client in background
# 3. Wait random time for transfer (between 1 and $TRANS_WAIT seconds)
# 4. Stop Client
# 5. Ensure Server shut down automatically
run_test() {
	local host="${1}"
	local port="${2}"
	local srv_opts="${3:-}"
	...
	# --------------------------------------------------------------------------------
	# TEST: Server stayed alive
	# --------------------------------------------------------------------------------
	echo;print_h2 "(5/5) Test: Server stays alive after Client quits"
