...
print_test_case "[206] Basics: (TCP) Client send command to Server (${PYVER})"
...
for i in $(seq "${RUNS}"); do
	...
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vvvv" "-vvvv" "${i}" "1"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vvv " "-vvvv" "${i}" "2"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vv  " "-vvvv" "${i}" "3"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -v   " "-vvvv" "${i}" "4"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash      " "-vvvv" "${i}" "5"
	RPORT=$(( RPORT + 1))

	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vvvv" "-vvv " "${i}" "6"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vvvv" "-vv  " "${i}" "7"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vvvv" "-v   " "${i}" "8"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vvvv" "     " "${i}" "9"
	RPORT=$(( RPORT + 1))

	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vvv " "-vvv " "${i}" "10"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -vv  " "-vv  " "${i}" "11"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash -v   " "-v   " "${i}" "12"
	RPORT=$(( RPORT + 1))
	run_test "${RHOST}" "${RPORT}" "-e /bin/bash      " "     " "${i}" "13"
	RPORT=$(( RPORT + 1))
done
