...
print_test_case "[204] Basics: (TCP) Client send file to Server (${PYVER})"
...
run_test() {
	...
	# shellcheck disable=SC2086
	cli_pid="$( run_bg "cat ${data_file}" "${PYTHON}" "${BINARY}" ${cli_opts} "${host}" "${port}" "${cli_stdout}" "${cli_stderr}" )"
	...
	cnt=0
	while ! diff "${data_file}" "${srv_stdout}" >/dev/null 2>&1; do
		printf "."
		cnt=$(( cnt + 1 ))
