...
for i in $(seq "${RUNS}"); do
	echo
	run_test "-n -u        ${PWNCAT_OPTS}" "${i}" "1"
	run_test "-n -u -vvvv  ${PWNCAT_OPTS}" "${i}" "2"
	run_test "-n -u -vvv   ${PWNCAT_OPTS}" "${i}" "3"
	run_test "-n -u -vv    ${PWNCAT_OPTS}" "${i}" "4"
	run_test "-n -u -v     ${PWNCAT_OPTS}" "${i}" "5"
	run_test "-n -u        ${PWNCAT_OPTS}" "${i}" "6"
done
