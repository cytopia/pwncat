...
run_test() {
	...
	while ! diff <(printf "${expect}" | od -c) <(od -c "${cli_stdout}") >/dev/null 2>&1; do
		printf "."
		cnt=$(( cnt + 1 ))
		if [ "${cnt}" -gt "${TRANS_WAIT}" ]; then
			...
			diff <(printf "${expect}" | od -c) <(od -c "${cli_stdout}") 2>&1 || true
	...
	print_data "Client received data" "$( od -c "${cli_stdout}" )"
