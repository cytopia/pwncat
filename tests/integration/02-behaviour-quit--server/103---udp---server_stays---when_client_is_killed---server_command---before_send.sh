...
for curr_round in $(seq "${RUNS}"); do
	...
	run_test "-l ${RPORT}    -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -4 -u -vvvv"  "2" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT}    -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -6 -u -vvvv"  "3" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RPORT} -4 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT}    -u -vvvv"  "4" "14" "${curr_round}" "${RUNS}"
	...
	run_test "-l ${RHOST} ${RPORT} -4 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT}    -u -vvvv"  "11" "14" "${curr_round}" "${RUNS}"
