...
run_test() {
	...
	# --------------------------------------------------------------------------------
	print_h2 "(4/5) Test: Server stays alive"
	run "sleep 2"
...
for curr_round in $(seq "${RUNS}"); do
	...
	# BIND ON ANY
	run_test "-l ${RPORT}    -e /bin/sh -u -vvvv" "${RHOST} ${RPORT}    -u -vvvv"  "1" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT}    -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -4 -u -vvvv --udp-sconnect --udp-sconnect-word"  "2" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT}    -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -6 -u -vvvv"  "3" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RPORT} -4 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT}    -u -vvvv --udp-sconnect --udp-sconnect-word"  "4" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT} -4 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -4 -u -vvvv"  "5" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RPORT} -6 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT}    -u -vvvv"  "6" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT} -6 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -6 -u -vvvv"  "7" "14" "${curr_round}" "${RUNS}"

	# BIND ON SPECIFIC
	run_test "-l ${RHOST} ${RPORT}    -e /bin/sh -u -vvvv" "${RHOST} ${RPORT}    -u -vvvv"   "8" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT}    -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -4 -u -vvvv"   "9" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT}    -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -6 -u -vvvv"  "10" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RHOST} ${RPORT} -4 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT}    -u -vvvv --udp-sconnect --udp-sconnect-word"  "11" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT} -4 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -4 -u -vvvv"  "12" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RHOST} ${RPORT} -6 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT}    -u -vvvv"  "13" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT} -6 -e /bin/sh -u -vvvv" "${RHOST} ${RPORT} -6 -u -vvvv"  "14" "14" "${curr_round}" "${RUNS}"
done
