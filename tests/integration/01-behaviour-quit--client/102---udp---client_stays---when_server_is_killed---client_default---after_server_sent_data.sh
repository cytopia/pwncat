...
run_test() {
	...
	# UDP client must send something, otherwise the server doesn't know it is there
	# shellcheck disable=SC2086
	if ! cli_pid="$( run_bg "echo ' '" "${PYTHON}" "${BINARY}" ${cli_opts} "${cli_stdout}" "${cli_stderr}" )"; then
		printf ""
	fi
	...
	# DATA TRANSFER
	# --------------------------------------------------------------------------------
	print_h2 "(3/6) Transfer: Server -> Client"

	# [SERVER] Wait for data
	print_info "Wait for data transfer"
	cnt=0
	while ! diff <(echo "${data}") "${cli_stdout}" >/dev/null 2>&1; do
		printf "."
		cnt=$(( cnt + 1 ))
		if [ "${cnt}" -gt "${TRANS_WAIT}" ]; then
			...
			diff <(echo "${data}") "${cli_stdout}" 2>&1 || true
