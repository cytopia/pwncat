...
run_test() {
	...
	cnt=0
	while ! diff <(echo "${expect}") "${srv_stdout}" >/dev/null 2>&1; do
		printf "."
		cnt=$(( cnt + 1 ))
		if [ "${cnt}" -gt "${TRANS_WAIT}" ]; then
			...
			diff <(echo "${expect}") "${srv_stdout}" 2>&1 || true
			...
			print_error "[Receive Error] Received data on Server does not match expected response from Client"
			exit 1
		fi
		sleep 1
	done
