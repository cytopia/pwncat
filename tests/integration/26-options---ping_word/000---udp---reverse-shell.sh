...
run_test() {
	...
	# --------------------------------------------------------------------------------
	# TEST: RECEIVED PING WORD
	# --------------------------------------------------------------------------------
	print_h2 "(4/5) Transfer: Client -> Server (ping word)"

	# [SERVER -> Client]
	wait_for_data_transferred "^test(test)*" "" "" "Server" "${srv_pid}" "${srv_stdout}" "${srv_stderr}" "Client" "${cli_pid}" "${cli_stdout}" "${cli_stderr}"


	# --------------------------------------------------------------------------------
	# STOP: SERVER/CLIENT
	# --------------------------------------------------------------------------------
	print_h2 "(5/5) Stop: Server/Client"
...
for curr_round in $(seq "${RUNS}"); do
	...
	run_test "-l ${RPORT} --no-shutdown -u -vvvv" "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vvvv"  "1" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u -vvv " "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vvvv"  "2" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u -vv  " "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vvvv"  "3" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u -v   " "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vvvv"  "4" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u      " "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vvvv"  "5" "13" "${curr_round}" "${RUNS}"

	#run_test "-l ${RPORT} --no-shutdown -u -vvvv" "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vvv "  "6" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u -vvvv" "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vv  "  "7" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u -vvvv" "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -v   "  "8" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u -vvvv" "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u      "  "9" "13" "${curr_round}" "${RUNS}"

	#run_test "-l ${RPORT} --no-shutdown -u -vvv " "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vvv " "10" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u -vv  " "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -vv  " "11" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u -v   " "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u -v   " "12" "13" "${curr_round}" "${RUNS}"
	#run_test "-l ${RPORT} --no-shutdown -u      " "${RHOST} ${RPORT} --no-shutdown --ping-word test --ping-intvl 1 -u      " "13" "13" "${curr_round}" "${RUNS}"
done
