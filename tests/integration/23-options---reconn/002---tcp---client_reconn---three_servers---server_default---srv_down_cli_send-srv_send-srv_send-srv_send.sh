...
run_test() {
	...
	# START: CLIENT
	# --------------------------------------------------------------------------------
	print_h2 "(1/12) Start: Client (round 1)"
	...
	# --------------------------------------------------------------------------------
	# [ROUND-1 SRV-SEND] START: SERVER
	# --------------------------------------------------------------------------------
	print_h2 "(2/12) Start: Server (round 1) (sends data to Client)"

	# Start Server
	print_info "Start Server"
	# shellcheck disable=SC2086
	if ! srv_pid="$( run_bg "printf ${data}" "${PYTHON}" "${BINARY}" ${srv_opts} "${srv1_stdout}" "${srv1_stderr}" )"; then
		printf ""
	fi
	...
	# --------------------------------------------------------------------------------
	# [ROUND-1: SRV-SEND] DATA TRANSFER
	# --------------------------------------------------------------------------------
	...
	# [SERVER -> Client]
	wait_for_data_transferred "" "${data}" "${data_or}" "Client" "${cli_pid}" "${cli_stdout}" "${cli_stderr}" "Server" "${srv_pid}" "${srv1_stdout}" "${srv1_stderr}"


	# --------------------------------------------------------------------------------
	# [ROUND-1]: SRV-NOSEND] STOP: SERVER
	# --------------------------------------------------------------------------------
	print_h2 "(5/12) Stop: Server (round 1)"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(6/12) Start: Server (round 2) (sends data to Client)"
	...
	# [SERVER -> Client]
	wait_for_data_transferred "" "${data}${data}" "${data_or}${data_or}" "Client" "${cli_pid}" "${cli_stdout}" "${cli_stderr}" "Server" "${srv_pid}" "${srv2_stdout}" "${srv2_stderr}"


	# --------------------------------------------------------------------------------
	# [ROUND-2]: SRV-SEND] STOP: SERVER
	# --------------------------------------------------------------------------------
	print_h2 "(8/12) Stop: Server (round 2)"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(9/12) Start: Server (round 3) (sends data to Client)"
	...
	# [SERVER -> Client]
	wait_for_data_transferred "" "${data}${data}${data}" "${data_or}${data_or}${data_or}" "Client" "${cli_pid}" "${cli_stdout}" "${cli_stderr}" "Server" "${srv_pid}" "${srv3_stdout}" "${srv3_stderr}"


	# --------------------------------------------------------------------------------
	# [ROUND-3]: SRV-SEND] STOP: SERVER
	# --------------------------------------------------------------------------------
	print_h2 "(11/12) Stop: Server (round 3)"
...
for curr_round in $(seq "${RUNS}"); do
	...
	# BIND ON ANY
	run_test "-l ${RPORT}    -vvvv" "${RHOST} ${RPORT}    --reconn -vvvv"  "1" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT}    -vvvv" "${RHOST} ${RPORT} -4 --reconn -vvvv"  "2" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT}    -vvvv" "${RHOST} ${RPORT} -6 --reconn -vvvv"  "3" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RPORT} -4 -vvvv" "${RHOST} ${RPORT}    --reconn -vvvv"  "4" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT} -4 -vvvv" "${RHOST} ${RPORT} -4 --reconn -vvvv"  "5" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RPORT} -6 -vvvv" "${RHOST} ${RPORT}    --reconn -vvvv"  "6" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT} -6 -vvvv" "${RHOST} ${RPORT} -6 --reconn -vvvv"  "7" "14" "${curr_round}" "${RUNS}"

	# BIND ON SPECIFIC
	run_test "-l ${RHOST} ${RPORT}    -vvvv" "${RHOST} ${RPORT}    --reconn -vvvv"   "8" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT}    -vvvv" "${RHOST} ${RPORT} -4 --reconn -vvvv"   "9" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT}    -vvvv" "${RHOST} ${RPORT} -6 --reconn -vvvv"  "10" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RHOST} ${RPORT} -4 -vvvv" "${RHOST} ${RPORT}    --reconn -vvvv"  "11" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT} -4 -vvvv" "${RHOST} ${RPORT} -4 --reconn -vvvv"  "12" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RHOST} ${RPORT} -6 -vvvv" "${RHOST} ${RPORT}    --reconn -vvvv"  "13" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT} -6 -vvvv" "${RHOST} ${RPORT} -6 --reconn -vvvv"  "14" "14" "${curr_round}" "${RUNS}"
done
