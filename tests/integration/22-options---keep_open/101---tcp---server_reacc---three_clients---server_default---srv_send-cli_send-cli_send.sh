...
run_test() {
	...
	# [SERVER -> Client]
	wait_for_data_transferred "" "${data}" "Client" "${cli_pid}" "${cli1_stdout}" "${cli1_stderr}" "Server" "${srv_pid}" "${srv_stdout}" "${srv_stderr}"
	...
	# [CLIENT -> SERVER]
	wait_for_data_transferred "" "${data}" "Server" "${srv_pid}" "${srv_stdout}" "${srv_stderr}" "Client" "${cli_pid}" "${cli2_stdout}" "${cli2_stderr}"
	...
	# [CLIENT -> SERVER]
	wait_for_data_transferred "" "${data}${data}" "Server" "${srv_pid}" "${srv_stdout}" "${srv_stderr}" "Client" "${cli_pid}" "${cli3_stdout}" "${cli3_stderr}"
