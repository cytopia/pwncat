...
run_test() {
	...
	# --------------------------------------------------------------------------------
	print_h2 "(2/12) Start: Client (round 1)"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(3/12) Stop: Client (round 1)"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(4/12) Test: Server stays alive (round 1)"
	run "sleep 2"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(5/12) Start: Client (round 2)"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(6/12) Stop: Client (round 2)"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(7/12) Test: Server stays alive (round 2)"
	run "sleep 2"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(8/12) Start: Client (round 3)"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(10/12) Stop: Client (round 3)"
	...
	# --------------------------------------------------------------------------------
	print_h2 "(11/12) Test: Server stays alive (round 3)"
	run "sleep 2"
...
for curr_round in $(seq "${RUNS}"); do
	...
	# BIND ON ANY
	run_test "-l ${RPORT} --no-shutdown    -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown    -vvvv"  "1" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT} --no-shutdown    -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown -4 -vvvv"  "2" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT} --no-shutdown    -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown -6 -vvvv"  "3" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RPORT} --no-shutdown -4 -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown    -vvvv"  "4" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT} --no-shutdown -4 -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown -4 -vvvv"  "5" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RPORT} --no-shutdown -6 -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown    -vvvv"  "6" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RPORT} --no-shutdown -6 -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown -6 -vvvv"  "7" "14" "${curr_round}" "${RUNS}"

	# BIND ON SPECIFIC
	run_test "-l ${RHOST} ${RPORT} --no-shutdown    -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown    -vvvv"   "8" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT} --no-shutdown    -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown -4 -vvvv"   "9" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT} --no-shutdown    -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown -6 -vvvv"  "10" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RHOST} ${RPORT} --no-shutdown -4 -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown    -vvvv"  "11" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT} --no-shutdown -4 -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown -4 -vvvv"  "12" "14" "${curr_round}" "${RUNS}"

	run_test "-l ${RHOST} ${RPORT} --no-shutdown -6 -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown    -vvvv"  "13" "14" "${curr_round}" "${RUNS}"
	run_test "-l ${RHOST} ${RPORT} --no-shutdown -6 -e /bin/sh --keep-open -vvvv" "${RHOST} ${RPORT} --no-shutdown -6 -vvvv"  "14" "14" "${curr_round}" "${RUNS}"
done
