...
run_test() {
	...
	cnt=0
	while ! diff <(echo "${expect}") "${cli2_stdout}" >/dev/null 2>&1; do
		printf "."
		cnt=$(( cnt + 1 ))
		if [ "${cnt}" -gt "${TRANS_WAIT}" ]; then
			...
			diff <(echo "${expect}") "${cli2_stdout}" 2>&1 || true
			kill_pid "${cli_pid}" || true
			kill_pid "${srv_pid}" || true
			print_data "RECEIVED RAW" "$( od -c "${cli2_stdout}" )"
	...
	echo
	print_file "Client received data" "${cli2_stdout}"
